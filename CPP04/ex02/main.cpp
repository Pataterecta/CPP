...
int main()
{
	...
	// *** to the corrector ->>
	// Animal animatos; 
	// << just uncomment it to see if this line can make u compile or not is
	// the object of the exo02,, what a surprise
	// just to make a pure virture function and make the abstract class
	// wow.... ***
	...
	std::cout << "**********just some extra2********" << std::endl;
	Cat* halaCat = new Cat(); // Dog 으로 넣으면 작동 안된다구~ 근데 dynamic cast 없이 하면 UB 에러 각이어도 작동은 하더라
	hapiCat-> makeSound();
	Cat *walaCat (halaCat);
	if (walaCat)
	{
		walaCat->getBrain()->setIdea(0, "im happycat");
		walaCat->setIdea(2, "say hi");
		std::cout << walaCat->getIdea(0) << std::endl;
		std::cout << walaCat->getIdea(2) << std::endl;
		walaCat-> makeSound();
		...
		delete walaCat; // just to show that only pointer for the add so same as hapiCat effect
