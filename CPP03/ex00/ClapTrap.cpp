...
ClapTrap::ClapTrap() : Hit_p(10), Energy_p(10), A_dam(0)
{
	std::cout << "Default constructor called" << std::endl;
}

ClapTrap::ClapTrap(std::string Name) : Name(Name), Hit_p(10), Energy_p(10), A_dam(0)
{
...
ClapTrap::~ClapTrap()
{
	...
	std::cout << "ByeBye " << "ClapTrap " << this->Name << std::endl;
}
...
void ClapTrap::beRepaired(unsigned int amount)
{
	...
	std::string rep_str;
	ss3 >> rep_str;
	if (amount + this->Hit_p > 10)
	{
		std::cout << this->Name << " tried a lot for " << rep_str << " points but Heal max to 10 tho!(hit points repaired to 10)" << std::endl;
		this->Hit_p = 10;
		return ;
	}
	else
